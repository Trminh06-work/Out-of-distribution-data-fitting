...
Many studies have been contributed to showing \cite{gulrajani2020searchlostdomaingeneralization} distributional shifts in real data are roadblock to the ML/DL applications to other fields. In Computer Vision, \cite{taori2020measuringrobustnessnaturaldistribution} showed that recent image classifiers fail to OOD generalization
...
Regarding Natural Language Processing, \cite{ribeiro-etal-2020-beyond} proposed a benchmark, called CheckList, that leads to failure in semantic analysis due to tiny variations in the texts.
\\

\noindent
Although (\cite{kolesnikov2023wildtabbenchmarkoutofdistributiongeneralization}, \cite{Borisov_2024}) tabular data is the primary structure to retain data for training models in numerous areas, e.g. finance (\cite{heaton2018deep}, \cite{ozbayoglu2020deeplearningfinancialapplications}),
...
\textbf{Comment (delete after finished)}: this is a benchmark report, only cite the papers contributing to benchmarks.
\\
