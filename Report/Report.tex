...
Therefore, this paper serves as a benchmark consisting of various train/test splitting strategies on real datasets to evaluate model's capability towards OOD generalization on tabular regression tasks. The key contributions of our work are:

\begin{itemize}
  \item Introduction to a comprehensive benchamrk consisting of 29 real-world low dimensional (up to 20 features) tabular datasets specifically for regression tasks.
  These datasets are subsequently partitioned into several train/test pairs aiming for evaluating the models' OOD generalization. To our knowledge, 
  this marks the first benchamrk that encompasses certain approaches to split datasets for OOD extrapolation assessment.
  \item Conduct an experimental study to quantify and rank the robustness of a broad spectrum of standard ML/DL models on unseen distributions.
  \item Give insightful analysis to explore what characteristics of the proposed ML/DL methods impact their performance when generalising to unseen data regions.
\end{itemize}
...
Meanwhile, there are contributions, though still very limited, to benchmarking models on OOD data, based on distributional shifts, in tabular regression tasks, e.g. \textbf{Wild-Tab} \cite{kolesnikov2023wildtabbenchmarkoutofdistributiongeneralization}, \textbf{Shifts Dataset} \cite{malinin2021shifts}, and an unnamed benchmark provided by \cite{grinsztajn2022treebasedmodelsoutperformdeep}.
\\
...
Before going into the Definitions, let first recall the definition of regression.

\subsection{Definition 1. Distributional shifts}

\subsection{Definition 2. Convex hull of dataset}
...
\section{Notation and Abbreviation Reference List}
...
$\mathbf P^{\text{tr}}(\cdot), \ \mathbf P^{\text{te}}(\cdot)$ & Distribution of train and test data \\
$\mathbf P(\mathbf X), \ \mathbf P(\mathbf y)$ & Marginal Distribution \\
$\mathbf P(\mathbf y \mid \mathbf X)$ & Conditional Distribution \\
$(\mathbf x_i, y_i) \sim \mathbf P(\cdot)$ & A data point sampled from a distribution $\mathbf P(\cdot)$ \\
$f(\mathbf x_i) \mapsto y_i$ & True mapping from $\mathbf x_i \in \mathbf X$ onto $y_i \in \mathbf y$ \\
$\hat y_i = \hat f(\cdot) \approx y_i$ & An approximation mapping/function \\
$\ell (f(\mathbf x_i), y_i) \mapsto \mathbb R_{\ge 0}$ & loss of a single pair input/true output $(\mathbf x_i, y_i)$ \\
\hline
\end{tabularx}}
